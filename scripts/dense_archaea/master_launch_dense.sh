...
first_species=${archaeas[4]}
job_id=$(
    qsub \
    -v SPECIES=$first_species \
    -N dense_archaea_4 \
    -o /home/eliott.tempez/dense_output_$first_species.log \
    -e /home/eliott.tempez/dense_error_$first_species.log \
    -q lowprio \
...
for ((i = 5; i < ${#archaeas[@]}; i++)); do
    ...
    job_id=$(
        ...
        -q lowprio \
        -l ncpus=8 -l host=node04 -l mem=32gb -l walltime=50:00:00 \
        run_dense.sh
        )
done